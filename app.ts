  ...
  try {
    ...
    if (!oauth2Client) {
      return res.json({
        configured: false,
        authenticated: false,
        message: 'Google OAuth no está configurado (faltan CLIENT_ID o CLIENT_SECRET en Vercel).'
      });
    }
  ...
  try {
    ...
    if (!oauth2Client) {
      return res.status(400).json({
        error: 'OAuth no configurado en Vercel. Agrega CLIENT_ID y CLIENT_SECRET en Environment Variables de tu proyecto en Vercel (Settings > Environment Variables) y haz un Redeploy.',
        missingEnvVars: true
      });
    }
